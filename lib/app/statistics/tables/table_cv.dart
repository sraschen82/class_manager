...
DataTable cvTable(List<Classes> classes) {
  ...
  List<DataRow> rows() {
    return List.generate(
        ...
        (index) => DataRow(cells: [
              ...
              DataCell(Center(
                  child: Stats.cv(filterStudentsClass(classes[index]), 0)
                          .isFinite
                      ? Text(
                          '${Stats.cv(filterStudentsClass(classes[index]), 0).toStringAsFixed(0)}%')
                      : const Text(''))),
              DataCell(Center(
                  child: Stats.cv(filterStudentsClass(classes[index]), 1)
                          .isFinite
                      ? Text(
                          '${Stats.cv(filterStudentsClass(classes[index]), 1).toStringAsFixed(0)}%')
                      : const Text(''))),
              DataCell(Center(
                  child: Stats.cv(filterStudentsClass(classes[index]), 2)
                          .isFinite
                      ? Text(
                          '${Stats.cv(filterStudentsClass(classes[index]), 2).toStringAsFixed(0)}%')
                      : const Text(''))),
            ]));
  }
